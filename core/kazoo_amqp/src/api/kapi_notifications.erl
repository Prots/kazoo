...
-spec api_definitions() -> kapi_definitions().
api_definitions() ->
    [notify_update_definition()
    ,skel_definition()
    ,low_balance_definition()
...
api_definition(Name) when is_list(Name) ->
    api_definition(kz_term:to_binary(Name));
api_definition(?NE_BINARY=Name) ->
    DefinitionFun = kz_term:to_atom(<<Name/binary, "_definition">>),
    DefinitionFun().
...
bind_to_q(Q, ['new_fax'|T]) ->
    InboundBinding = kz_api:definition_binding(inbound_fax_definition()),
    OutboundBinding = kz_api:definition_binding(outbound_fax_definition()),
...
bind_to_q(Q, ['fax_error'|T]) ->
    InboundBinding = kz_api:definition_binding(inbound_fax_error_definition()),
    OutboundBinding = kz_api:definition_binding(outbound_fax_error_definition()),
    'ok' = amqp_util:bind_q_to_notifications(Q, InboundBinding),
    'ok' = amqp_util:bind_q_to_notifications(Q, OutboundBinding),
    bind_to_q(Q, T);
bind_to_q(Q, [RestrictTo|T]) ->
    try [kz_api:definition_binding(Definition)
         || Definition <- api_definitions(),
            kz_api:definition_restrict_to(Definition) =:= RestrictTo
        ]
    of
        [Binding] ->
...
headers(Name) ->
    try api_definition(Name) of
        Definition ->
            kz_api:definition_required_headers(Definition) ++ kz_api:definition_optional_headers(Definition)
    catch
        error:undef ->
...
%% @end
%%--------------------------------------------------------------------
-spec build_message(api_terms(), kapi_definition()) -> api_formatter_return().
